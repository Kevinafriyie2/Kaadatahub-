...
function osc_verify_afa_paystack_payment() {
    ...
    try {
        ...
        foreach ($registration_data as $key => $value) {
            $order->update_meta_data('_afa_' . $key, $value);
        }
...
function osc_process_afa_registration(){
    ...
    try {
        ...
        $order->update_meta_data('_afa_full_name', $full_name);
        $order->update_meta_data('_afa_phone_number', $phone_number);
        $order->update_meta_data('_afa_ghana_card', $ghana_card);
        $order->update_meta_data('_afa_location', $location);
        $order->update_meta_data('_afa_notes', $notes);
