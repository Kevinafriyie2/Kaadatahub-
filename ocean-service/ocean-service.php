...
function osc_network_products_page(){
    ...
    // Handle Add Network
    if(isset($_POST['osc_add_network_submit']) && check_admin_referer('osc_add_network', 'osc_add_network_nonce')){
        $new_network_name = sanitize_text_field($_POST['new_network_name']);
        if(!empty($new_network_name)){
            ...
            if(!isset($network_products[$new_network_name])){
                ...
                echo '<div class="notice notice-success is-dismissible"><p>Network "'.esc_html($new_network_name).'" added.</p></div>';
            } else {
                echo '<div class="notice notice-warning is-dismissible"><p>Network "'.esc_html($new_network_name).'" already exists.</p></div>';
            }
        }
    }

    // Handle Delete Network
    if(isset($_POST['osc_delete_network']) && check_admin_referer('osc_delete_network_nonce_' . sanitize_key($_POST['osc_delete_network']))){
        $network_to_delete = sanitize_text_field($_POST['osc_delete_network']);
        $network_products = get_option('osc_network_products', []);
        if(isset($network_products[$network_to_delete])){
            unset($network_products[$network_to_delete]);
            update_option('osc_network_products', $network_products);
            echo '<div class="notice notice-success is-dismissible"><p>Network "'.esc_html($network_to_delete).'" deleted.</p></div>';
        }
    }
...
<div class="wrap">
    ...
    <div class="card">
        ...
        <form method="post">
            <?php wp_nonce_field('osc_add_network', 'osc_add_network_nonce'); ?>
            <input type="text" name="new_network_name" placeholder="Enter new network name" required>
            <button type="submit" name="osc_add_network_submit" class="button button-primary">Add Network</button>
        </form>
    </div>
            ...
            <tr>
                <th scope="row" style="width: 200px;">
                    ...
                    <form method="post" style="display:inline; margin-left: 10px;">
                        <?php wp_nonce_field('osc_delete_network_nonce_' . sanitize_key($network)); ?>
                        <input type="hidden" name="osc_delete_network" value="<?php echo esc_attr($network); ?>">
                        <button type="submit" class="button is-destructive" onclick="return confirm('Are you sure you want to delete this network?');">Delete</button>
                    </form>
                </th>
                <td>
...
<script>
    jQuery(function($) {
        // Ensure wc-product-search select boxes are initialized
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.length) {
                    ...
                    bundleSelect.append('<option>No bundles found</option>').prop('disabled', true);
                }
            }
        });
    });
